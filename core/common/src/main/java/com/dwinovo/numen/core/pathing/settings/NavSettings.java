...
public final class NavSettings {
     ...
     *
     * <p>12 ≈ 多走 2.6 格。她是住在别人世界里的客人:破坏该是绕不开时的下策,不是抄
     * 近道的手段 —— 但 30(≈多走 6.5 格)把这个"下策"推得太远了:一堵两格厚的墙
     * (脚+头两格)要 60 徒手叠加,而绕过去只要十几格,于是她宁可贴着墙走出一条长弯
     * 路也不拆那两块,玩家看到的是"明明能穿过去,却绕了半圈"。该保的东西现在交给
     * <b>硬禁</b>去保(do_not_break 标签、玩家自己放过的方块、岩浆/落沙邻格、
     * 世界边界),软惩罚只留一点"能绕就绕"的倾向。
     * 参照系:挖掘型机器人拿这个值当平手判定(≈2,工具越好拆墙越接近免费),
     * 定居型 NPC 根本没有破坏这个选项(∞)——同伴两头的活都要干,取中段。
     */
    public double blockBreakAdditionalPenalty = 12.0;

    /**
     * 挖一个<b>紧挨着水</b>的方块时的成本乘数(默认 4)。
     *
     * <p>邻格有水曾经是硬禁挖(COST_INF):挖穿之后水会顺着缺口灌下来,淹掉一段通道、
     * 把她冲离路径。但硬禁逼出来的绕路往往更长,而地下挖矿时脚边有水是常态(矿井、
     * 含水层、河道底下)——结果是该挖的矿碰都不碰。现在改成"贵但可挖"的倍数罚金。
     *
     * <p>真正要命的邻格(岩浆、悬空落沙)仍然是 COST_INF,一点没松:水只是把她冲走,
     * 岩浆和塌方是要命的。冰也不在此列——冰就是水,挖掉它只是开出一条路。
     */
    public double waterAdjacentBreakPenaltyMultiplier = 4.0;
    /** 每次起跳的附加罚金。 */
     ...
     * 能规划走不动"。本轮把那半个前提补上了:{@link com.dwinovo.numen.core.pathing.moves.Movement#waterDrive}
